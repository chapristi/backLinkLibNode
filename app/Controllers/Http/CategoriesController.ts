...
export default class CategoriesController {
  ...
  public async store({ bouncer, request, response }: HttpContextContract): Promise<void> {
    try {
      await bouncer.with('CatgoriesPolicy').authorize('create')
      ...
      return response.status(500).json({
        error: err.message,
        message: 'the server returned an error! Impossible to create a category! try again!',
      })
    }
  }
  ...
  public async update({ bouncer, params, response, request }: HttpContextContract): Promise<void> {
    try {
      ...
      const category: any[] = await Category.query().where('id', params.id).update(payload)
      response.ok({ category: category, message: 'the category was updated' })
    } catch (err: any) {
      return response.status(500).json({
        error: err.message,
        message: 'the server returned an error! Impossible to uodate a category! try again!',
      })
    }
  }
  ...
  public async destroy({ response, bouncer, params }: HttpContextContract): Promise<void> {
    try {
      ...
      return response
        .status(500)
        .json({ error: err.message, message: 'impossible to delete this category for the moment!' })
    }
  }
}
