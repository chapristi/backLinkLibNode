...
export default class PostsController {
  ...
  public async store({ request, response, auth }: HttpContextContract): Promise<void> {
    try {
      ...
      const post: Promise<Post> = Post.create({
        name: name,
        link: link,
        image: image,
      ...
      response.status(500).json({
        error: err.message,
        message: 'the server returned an error! Impossible to right a post! try again!',
      })
    }
  }
  ...
  public async update({ request, params, response, bouncer }: HttpContextContract): Promise<void> {
    try {
      ...
      return response.status(500).json({
        err: err.message,
        message: 'The server returned an error!Impossible to update this post',
      })
    }
  }
